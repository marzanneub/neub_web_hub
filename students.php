...
<?php
	...
	if($found)
	{
		while($row = mysqli_fetch_assoc($sql_query))
		{
			echo "<tr><td><img width='70' height='70' src='data:image/jpeg;base64," . base64_encode($row['photo']) . "'></td><td>" . $row["s_id"] . "</td><td>" . $row["name"] . "</td><td>" . $row["prog_name"] . "</td><td>" . $row["dept_name"] . "</td><td>" . $row["session_name"] . "</td><td>" . $row["phone_number"] . "</td><td>" . $row["address"] . "</td><td>" . $row["email"] . "</td></tr>";
		}
	}
?>
</table>
