...
require('auto_login.php');
	...
	if($prof=='s')
	{
		...
		else
		{
			...
			$sql = "UPDATE student SET password = '$pw1', login = '1', registration = '1' WHERE s_id = '$id'";
			$sql_query = mysqli_query($con, $sql);
			
			if($sql_query)
			{
				$message="Successfully registered";
				header("Location: student_landing.php?reg_message=$message&s_id=$id");
				exit();
			}
			else
			{
	...
	if($prof=='t')
	{
		...
		else
		{
			...
			$sql = "UPDATE teacher SET password = '$pw1', login = '1', registration = '1' WHERE t_id = '$id'";
			$sql_query = mysqli_query($con, $sql);
			
			if($sql_query)
			{
				$message="Successfully registered";
				header("Location: teacher_landing.php?reg_message=$message&t_id=$id");
				exit();
			}
			else
			{
